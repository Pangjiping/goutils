...
func TestOrderedMapJSON(t *testing.T) {
	...
	t.Run("MarshalJSON", func(t *testing.T) {
		...
		json_, err := json.Marshal(m)
		if err != nil {
			t.Fatal(err)
		}
		reflect.DeepEqual(`{"string":"foo","number":1337,"slice":["foo","bar"],"map":{"foo":"bar"},"bool":true,"nil":null,"struct":{"foo_tag":"bar"},"orderedMap":{"string":"foo","number":1337,"slice":["foo","bar"]}}`, string(json_))
	})
